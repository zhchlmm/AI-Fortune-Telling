...
function extractInputValue(event: unknown): string | null {
  ...
  if (detail && typeof detail === 'object') {
    const detailValue = (detail as { value?: unknown }).value
    if (typeof detailValue === 'string') {
      return detailValue
    }

    const currentValue = (detail as { current?: unknown }).current
...
Component({
  ...
  methods: {
    initFortuneTypeOptions() {
      const valueOptions = ((this.properties.fortuneTypeOptions as unknown as string[]) ?? []).filter(
    ...
    applyInitialFortuneType() {
      ...
      const valueOptions = this.data.fortuneTypeValueOptions as string[]
      const displayOptions = this.data.fortuneTypeDisplayOptions as string[]
    ...
    removePhoto(e: WechatMiniprogram.CustomEvent<{ index: string }>) {
    ...
    previewPhoto(e: WechatMiniprogram.CustomEvent<{ url: string }>) {
    ...
    setFailedPhotoPaths(paths: string[]) {
    ...
    onRetryFailedTap() {
      ...
      const detail: FortuneFormSubmitDetail = {
